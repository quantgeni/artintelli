...
# 회귀분석은 (상관분석 + 예측)하는 통계적 기법
# 수량형 값을 예측하는데 주로 사용되는 통계적 모형

# 두 변수간의 선형(직선)관계를 식으로 표현하고 독립변수가 주어지면
# 종속변수의 값이 얼마인지 추정

# 회귀방정식 : 독립변수와 종속변수사이의 관계를 선형방정식으로 나타낸 것
...
# 회귀직선을 그릴때는 최소제곱법이라는 수학적 기법을 이용
# 최소제곱법 : 두 변수의 선형관계를 계량화한 것

# R에서는 lm(종속변수~독립변수), predict() 함수를 이용함

# 회귀분석의 시초
# 영국 유전학자 골턴은 부모의 키와 자녀들의 키사이의 연관 관계를 조사 
# => 선형적 관계가 있음을 파악
# 후에 칼 피어슨이 이것을 수학적으로 증명
# => 회귀분석이론 정립

# => 앞으로 발생할 일은 지난 과거에 일어난 일의 [평균수준]으로 돌아가는 성질이 있음
# => 아버지의 키가 아무리 크더라도 아들의 키는 아버지 세대의 평균 키에 맞춰짐


# ex) 판촉전화횟수와 판매량 사이의 관계를 최소제곱법을 이용해서 선형방정식으로 
# 표현하고, 판촉전화를 100회 시행시 추정되는 판매량은 얼마인가?
# 전화횟수p / 판매량s
...
b(기울기) = cor(p,s) * (sd(s)/sd(p)) # 0.260625
a(절편) = mean(s)  - 0.260625*mean(p) # 19.98
# 기울기, 절편을 구하는 함수사용
lm(s ~ p) # (Intercept) : 19.9800 / p : 0.2606
# 작성된 선형방정식
y = a+bx = 19.98 + 0.261 * x
# a) 0.261 * x 의 의미 : 판촉전화 1회 시도 제품은 0.261대 판매됨
# 19.98 의 의미 : 판촉전화를 하지 않아도 자연적으로 판매되는 제품 수??
#                 판촉전화의 횟수 범위 : range(p) => 36
...
# 최소제곱법을 이용한 절편/기울기 계산
# 기울기a : sum((x - x평균) * (y - y평균)) / sum((x - x평균)^2)
# 절편b : y평균 - (x평균*기울기)
xbar <- mean(p)
ybar <- mean(s)
...
# ex) applewood 데이터를 기초로 해서 구매자 연령과 판매이익간의 관계를 
# 최소제곱법을 이용해서 선형방정식으로 나타내시오
setwd ('C:/Java/data/')
aw <- read.table('applewood.txt', sep=" ", header=T)
View(aw)
Age <- aw$Age
Profit <- aw$Profit
str(Profit)
...
# 산점도를 그리고, 상관계수와 회귀방정식을 구해서 매출액이 50일때 이익금액을 
# 추정하시오(smallbs.txt)
setwd ('C:/Java/data/')
...
# ex) 뮤추얼펀드에 투자하기 위해 자산과 5년간 실적을 조사하였다. 9개의 뮤추얼펀드를
# 임의로 추출하여 자산가치와 수익율을 토대로 산점도를 그리고, 상관계수와 
# 회귀방정식을 구해서 펀드투자액이 4억일때 5년 수익율은?(단위:백만) (funds.txt)
setwd ('C:/Java/data/')
...
# ex) 시의회에서 범죄발생율을 낮추기 위해 경찰관을 늘리고자 함. 다른 시의 경찰관
# 수와 범죄발생건수를 토대로 산점도를 그리고, 상관계수와 회귀방정식을 구해서 
# 경찰관 수가 20일때 범되발생율은? (citycrime.txt)
setwd ('C:/Java/data/')
...
# ex) 중고차의 연령과 가격 사이의 관계를 알고자 함. 작년에 판매된 중고차량 중 12대를 
# 기반으로 산점도를 그리고 상관계수와 회귀방정식을 구해서 중고차 연령이 12일때
...
# 예측구간 추정
# 추정치의 표준오차와 결정계수를 통해 회귀식의 종속변수 예측력에 대한 전반적인
# 평가를 내렸었음

# 회귀식의 예측력을 표현하는 또 다른 방식은 독립변수의 특정값을 신뢰구간으로
# 계산하여 제시
...
# ex) 전화횟수 대비 판매량에 대한 회귀식을 토대로 50회를 걸었을때 판매량의 95% 
# 신뢰구간과 예측구간을 구하시오
...
# ex) 식이요법을 적용한 닭에 대한 데이터 '1'번 닭을 대상으로 시간대비 몸무게에 대한 
# 회기분석을 실시하시오.(MASS패키지의 ChickWeight)
library(MASS)
str(ChickWeight)

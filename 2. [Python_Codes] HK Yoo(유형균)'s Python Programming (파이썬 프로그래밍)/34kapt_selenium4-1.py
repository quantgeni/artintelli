...
# 따라서, 2018, 08, 서울, 강남구, 삼성동을 
# 선택했을때 보여지는 코드값이 무엇인지 파악해봄
...
try:
    ...
    # 검색할 단지 정보 입력 2
    yy = Select(chrome.\
        find_element_by_class_name('combo_YYYY'))
    yy.select_by_visible_text('2018')
    time.sleep(1)

    mm = Select(chrome.\
        find_element_by_class_name('combo_MM'))
    mm.select_by_visible_text('08')
    time.sleep(1)

    sido = Select(chrome.\
        find_element_by_class_name('combo_SIDO'))
    sido.select_by_visible_text('서울특별시')
    time.sleep(1)

    gugun = Select(chrome.\
        find_element_by_class_name('combo_SGG'))
    gugun.select_by_visible_text('강남구')
    time.sleep(1)

    dong = Select(chrome.\
        find_element_by_class_name('combo_EMD'))
